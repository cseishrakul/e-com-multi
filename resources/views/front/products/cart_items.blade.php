                        ...
                        <div class="cart-anchor-image">
                            <a href="{{ url('product/' . $item['product_id']) }}">
                                <img src="{{ asset('admin/photos/product_images/small/' . $item['product']['product_image']) }}"
                                    alt="Product">
                                <h6>
                                    <b>{{ $item['product']['product_name'] }}
                                        ({{ $item['product']['product_code'] }})
                                        -
                                        {{ $item['size'] }}</b> <br>
                                    <b>Color: </b> {{ $item['product']['product_color'] }}
                                </h6>
                            </a>
                        </div>
                    </td>
                            ...
                            @if ($getDiscountAttributePrice['discount'] > 0)
                                <div class="price-template">
                                    <div class="item-new-price">
                                        {{ $getDiscountAttributePrice['final_price'] }} Tk.
                                    </div>
                                    <div class="item-old-price" style="margin-left: -55px">
                                        {{ $getDiscountAttributePrice['product_price'] }} Tk.
                                    </div>
                                </div>
                            @else
                                <div class="price-template">
